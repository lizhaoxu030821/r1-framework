...
private:
  ...
  // 19 DOF 扩展后，BridgeHW 即使暂未作为编译目标使用，也必须与底层
  // motorDate_recv/YKSMotorData 数组长度一致，避免后续重新启用时出现越界。
  YKSMotorData yksSendcmdzero_[ZKY_TOTAL_MOTORS] = {};
  YKSMotorData yksSendcmd_[ZKY_TOTAL_MOTORS];
  float transform_CurrentPos[ZKY_TOTAL_MOTORS] = { 0 };
  ...
  float baseMotor_[ZKY_TOTAL_MOTORS] = { 0 };
};
