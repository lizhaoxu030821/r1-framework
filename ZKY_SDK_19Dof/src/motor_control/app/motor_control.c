...
// 19 DOF 反馈索引映射说明：
// 从站0/1 仍然各有 6 个腿部电机；从站2 新增 7 个上肢/腰部电机。
// 反馈 CAN id 在每个从站内都会从 1 重新编号，因此不能再简单使用
// (slave_id * 6 + id - 1) 覆盖所有从站，否则从站2的 id7 会越界或丢失。
static int zky_feedback_offset_for_slave(uint8_t slave_id)
{
    if (slave_id == 0)
        return 0;
    if (slave_id == 1)
...
static int zky_motor_count_for_slave(uint8_t slave_id)
{
...
static int zky_feedback_index_from_motor_id(uint8_t slave_id, uint16_t motor_id)
{
    const int offset = zky_feedback_offset_for_slave(slave_id);
    const int count = zky_motor_count_for_slave(slave_id);
    if (offset < 0 || motor_id == 0 || motor_id > (uint16_t)count)
        return -1;
    return offset + (int)motor_id - 1;
}

static int zky_feedback_index_from_local_slot(uint8_t slave_id, int local_slot)
{
    const int offset = zky_feedback_offset_for_slave(slave_id);
    const int count = zky_motor_count_for_slave(slave_id);
    if (offset < 0 || local_slot < 0 || local_slot >= count)
        return -1;
    return offset + local_slot;
}
...
void send_motor_ctrl_cmd(EtherCAT_Msg *TxMessage, uint8_t data_channel, uint16_t motor_id, float kp, float kd, float pos, float spd, float tor)
{
    ...
    int spd_int;
    int tor_int;
    // data_channel 是从站 PDO 内的 CAN 帧槽位；从站2新增 body_joint 后需要第7槽。
    // 腿部从站仍只会通过限长 PDO 拷贝发送前6槽，不会写到旧从站映射外。
    if (data_channel < 1 || data_channel > ZKY_MAX_MOTORS_PER_SLAVE)
        return;
    TxMessage->can_ide = 0;
...
void RV_can_data_repack(EtherCAT_Msg *RxMessage, uint8_t comm_mode, uint8_t slave_id)
{
    ...
    static int last_print_counter = -1000;
    bool should_print = 0; // 是否打印
    // bool should_print = (debug_counter++ % 100 == 0); // 每100次打印一次
    // bool should_print = ((debug_counter - last_print_counter) >= 100);
    if (should_print && slave_id == 0){
        last_print_counter = debug_counter;
    }
    debug_counter++;
    ...
    if (should_print) {
        printf("[REPACK] Slave %d: comm_mode=0x%02X\n", slave_id, comm_mode);
    }
    
    for (int i = 0; i < ZKY_MAX_MOTORS_PER_SLAVE; ++i)
    {
        if (RxMessage->motor[i].dlc == 0) {
            if (should_print) {
                printf("[REPACK] Slave %d motor[%d]: dlc=0 (empty), skipping\n", slave_id, i);
            }
            continue;
        }

        if (should_print) {
            printf("[REPACK] Slave %d motor[%d]: id=0x%X, dlc=%d, comm_mode=0x%02X\n", 
                   slave_id, i, RxMessage->motor[i].id, RxMessage->motor[i].dlc, comm_mode);
        }

        if (RxMessage->motor[i].id == 0x7FF)
        {
            if (should_print) {
            printf("[REPACK] Slave %d motor[%d]: Entering 0x7FF branch\n", 
                   slave_id, i);
            }
        ...
        {
            if (should_print) {
            printf("[REPACK] Slave %d motor[%d]: Entering comm_mode==0x00 branch\n", 
                   slave_id, i);
            }
            ack_status = RxMessage->motor[i].data[0] >> 5; 
            const int motor_index = zky_feedback_index_from_motor_id(slave_id, RxMessage->motor[i].id);
            if (motor_index < 0 || motor_index >= TOTAL_MOTOR_NUMBER) {
                if (should_print) {
                    printf("[REPACK] Slave %d motor[%d]: invalid motor id %u for 19DOF map\n",
                           slave_id, i, RxMessage->motor[i].id);
                }
                continue;
            }
            motor_id_t = (uint8_t)motor_index;
            ...
            if (should_print) {
                printf("[REPACK] Slave %d motor[%d]: motor_id=%d (id_t=%d), ack_status=%d, dlc=%d\n", 
                       slave_id, i, motor_id_check, motor_id_t, ack_status, RxMessage->motor[i].dlc);
            }
            ...
            if (ack_status == 1) // response frame 1
            {
                ...
                if (should_print) {
                    printf("[REPACK] Slave %d: Parsed motor_id=%d -> rv_motor_msg[%d].angle=%.3f (pos_int=%d)\n", 
                           slave_id, motor_id_check, motor_id_t, 
                           rv_motor_msg[motor_id_t].angle_actual_rad, pos_int);
                }
            }
        ...
        {
            if (should_print) {
            printf("[REPACK] Slave %d motor[%d]: Entering comm_ode==0x01 branch\n", 
                   slave_id, i);
            }
            const int local_slot = (int)RxMessage->motor[i].id - 0x205;
            const int motor_index = zky_feedback_index_from_local_slot(slave_id, local_slot);
            if (motor_index < 0 || motor_index >= TOTAL_MOTOR_NUMBER) {
                if (should_print) {
                    printf("[REPACK] Slave %d motor[%d]: invalid auto-feedback id 0x%X for 19DOF map\n",
                           slave_id, i, RxMessage->motor[i].id);
                }
                continue;
            }
            ...
            if (should_print) {
            printf("[REPACK] Slave %d motor[%d]: id=0x%X, comm_mode=0x%02X, dlc=%d\n", 
                   slave_id, i, RxMessage->motor[i].id, comm_mode, RxMessage->motor[i].dlc);
            }
        }
    }
}
